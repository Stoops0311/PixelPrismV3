...
export function BrandSwitcher({ brands, currentBrand, onBrandChange }: BrandSwitcherProps) {
  ...
  function handleCreateBrandClick() {
    const maxBrands = user?.maxBrands ?? 1
    if (brands.length >= maxBrands) {
      upgrade.showUpgrade({ kind: "brands", currentCount: brands.length, maxCount: maxBrands })
      return
    }
  ...
  return (
    <>
      <DropdownMenu>
        ...
        >
          ...
          <DropdownMenuItem
            ...
            onSelect={handleCreateBrandClick}
          >
            <HugeiconsIcon
              icon={PlusSignIcon}
              strokeWidth={2}
      ...
      <CreateBrandDialog
        ...
        onSuccess={(slug) => onBrandChange(slug)}
      />
      <UpgradeDialog
        open={upgrade.open}
